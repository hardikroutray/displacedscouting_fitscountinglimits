...
for j in range(len(ctaus)):
        ...
        for i in INJ:
            ...
            for batch in range(1,nbatches+1):
                ...
                fitcmd0 = "combine -M FitDiagnostics -d %s -m %f -t %d --toysFile higgsCombine%s%f.GenerateOnly.mH%i.123456.root --rMin %f --rMax %f --saveWorkspace -n %s%f --forceRecreateNLL --keepFailures --saveNormalizations   --savePredictionsPerToy  --saveWithUncertainties --cminDefaultMinimizerStrategy=0" %(cardname, mass, ntoys, name, i, mass, rmin, rmax, name, i)

                fitcmd1 = "combine -M FitDiagnostics -d %s -m %f -t %d --toysFile higgsCombine%s%f.GenerateOnly.mH%.1f.123456.root --rMin %f --rMax %f --saveWorkspace -n %s%f --forceRecreateNLL --keepFailures --saveNormalizations   --savePredictionsPerToy  --saveWithUncertainties --cminDefaultMinimizerStrategy=0" %(cardname, mass, ntoys, name, i, mass, rmin, rmax, name, i)

                fitcmd2 = "combine -M FitDiagnostics -d %s -m %f -t %d --toysFile higgsCombine%s%f.GenerateOnly.mH%.2f.123456.root --rMin %f --rMax %f --saveWorkspace -n %s%f --forceRecreateNLL --keepFailures --saveNormalizations   --savePredictionsPerToy  --saveWithUncertainties --cminDefaultMinimizerStrategy=0" %(cardname, mass, ntoys, name, i, mass, rmin, rmax, name, i)

                fitcmd3 = "combine -M FitDiagnostics -d %s -m %f -t %d --toysFile higgsCombine%s%f.GenerateOnly.mH%.3f.123456.root --rMin %f --rMax %f --saveWorkspace -n %s%f --forceRecreateNLL --keepFailures --saveNormalizations   --savePredictionsPerToy  --saveWithUncertainties --cminDefaultMinimizerStrategy=0" %(cardname, mass, ntoys, name, i, mass, rmin, rmax, name, i)
            ...
            os.system("hadd fitDiagnosticscombined_r%i.root fitDiagnostics0*analysis%f.root" %(i,i))    
            
            # Plot
            # F = ROOT.TFile("fitDiagnostics%s%f.root" % (name, i))
            F = ROOT.TFile("fitDiagnosticscombined_r%i.root" % (i)) 
