...
bphiallcsv = pd.read_csv('totalacceptance_bphi.csv')
bphibincsv = pd.read_csv('binbybinacceptance_bphi.csv')
...
for j in range(len(masses)):
    ...
    for k in range(len(ctaus)):
        ...
        print "The acceptance of this mass,ctau point is ", acceptance(mass=masses[j],ctau=ctaus[k],sample="bphi",whichbin="allbins")
        acc_allbins = acceptance(mass=masses[j],ctau=ctaus[k],sample="bphi",whichbin="allbins")
        ...
        for l in range(len(lxybins)):
            for m in range(len(ptbins)):                                                      
                for n in range(len(isobins)):  
                    ...
                    acc_val = acceptance(mass=masses[j],ctau=ctaus[k],sample="bphi",whichbin="lxybin{}_ptbin{}_isobin{}".format(l+1,m+1,n+1)) 
        ...
        for file in glob.glob("simple*.txt"):
            ...
            trigsf_unc_val, oneminuseff_val = get_systematics(mass = masses[j], ctau = ctaus[k], sample="bphi")
            mcstat_unc_val = get_mcstatunc(mass = masses[j], ctau = ctaus[k], sample="bphi", whichbin=bin_val)
            ...
            singlebinlimits["bin_val"] = suffix

            for line in com_out:                                                                                                                                                       
        
                if line[:15] == 'Observed Limit:':                                                                                                                                  
                    singlebinlimits["r_obs"] = float(line[19:])                                                                                                                              

                elif line[:15] == 'Expected  2.5%:':                                                                                                                                
                    singlebinlimits["r_downdown"] = float(line[19:])                                                                                                                                 
                elif line[:15] == 'Expected 16.0%:':                                                                                                                                
                    singlebinlimits["r_down"] = float(line[19:])                                                                                                                                

                elif line[:15] == 'Expected 50.0%:':                                                                                                                                
                    singlebinlimits["r_exp"] = float(line[19:])                                                                                                                                 

                elif line[:15] == 'Expected 84.0%:':                                                                                                                                
                    singlebinlimits["r_up"] = float(line[19:])                                                                                                                                 
        ...
        brfrac = (coml_exp*totalsignalrate)/(101.3*2*acc_allbins*133000000000)
        brfracup = (coml_1su*totalsignalrate)/(101.3*2*acc_allbins*133000000000)
        brfracdown = (coml_1sd*totalsignalrate)/(101.3*2*acc_allbins*133000000000)
        brfracupup = (coml_2su*totalsignalrate)/(101.3*2*acc_allbins*133000000000)
        brfracdowndown = (coml_2sd*totalsignalrate)/(101.3*2*acc_allbins*133000000000)
        brfracobs = (coml_obs*totalsignalrate)/(101.3*2*acc_allbins*133000000000)
